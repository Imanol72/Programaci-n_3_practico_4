...
const updateUsuarioRol = async (req, res) => {
    try {
        ...
        // Validar que se mande un rol
        if (!rol) {
            return res.status(400).json({ status: 400, message: 'El rol es obligatorio' });
        }

        // Validar que el rol sea uno de los permitidos
        const rolesValidos = ['admin', 'moderador', 'cliente'];
        if (!rolesValidos.includes(rol)) {
            return res.status(400).json({ status: 400, message: `Rol inválido. Debe ser uno de: ${rolesValidos.join(', ')}` });
        }
        ...
        res.status(200).json({ status: 200, message: 'Rol de usuario actualizado exitosamente', data: usuario });
    } catch (error) {
        res.status(500).json({ status: 500, message: 'Error al actualizar rol de usuario', error: error.message });
    }
};
